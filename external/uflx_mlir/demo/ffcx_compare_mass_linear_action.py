...
`inner(w, v) * dx` where `w` is a Coefficient (not a TrialFunction) on a
CG-`degree` tetrahedron space -- the mass-operator sibling of
ffcx_compare_linear_action.py, which covers the same comparison for
`inner(grad(w), grad(v)) * dx` (the stiffness operator).

Tests degree down to 0, ie a DG0 (cellwise-constant) Coefficient: unlike
the stiffness operator, whose action is exactly zero for a DG0 Coefficient
(see uflx/operators.py's is_cellwise_constant handling and
test/test_dg0_coefficients.py), the MASS operator's action on a DG0
Coefficient is a genuine, nonzero single-dof result -- w's one dof times
the cell's volume-weighted "row sum" of test-function values -- so this is
the first place in this repo that runs a real, non-degenerate DG0
Coefficient through the full MLIR JIT pipeline and checks it numerically
against FFCx, rather than just checking (as the pure-uflx and codegen-level
tests do) that a DG0 Coefficient's *gradient* vanishes.

Two things needed changing relative to ffcx_compare_linear_action.py beyond
just dropping grad():

1. Element construction: a *continuous* Lagrange element of degree 0
   cannot be built at all (basix raises "Cannot create a continuous order
   0 Lagrange basis function") -- degree 0 must request the discontinuous
   variant. Continuity has no bearing on a single-cell local kernel like
   this one either way (only a global dofmap would ever observe it), so
   both build_*_form functions below pass discontinuous=(degree == 0)
   uniformly rather than only at degree 0 specifically, to keep the two
   sides' element constructions visibly parallel.

2. Quadrature sizing: uflx_mlir.lowering.lower_form (called by
   generate_mlir_module) sizes its quadrature rule as
   `max(2*(degree - 1), 1)` -- exactly right for a gradient-square
   (stiffness) integrand, but a mass integrand phi_i*phi_j has degree
   `2*degree`, which that formula under-integrates for every degree >= 1
   (checked directly against uflx_mlir.lowering.lower_form's own emitted
   quadrature-weights table while writing this file: eg degree=2 needs 14
   points but the stiffness-shaped formula only requests 4). Passing
   `degree + 1` as that function's `degree` argument instead gives
   `2*((degree+1) - 1) == 2*degree`, the exact rule -- see uflx_compile
   below and generate_kernel.reference_mass's own docstring, which notes
   the same fix for any future mass-type kernel built through this path.
   FFCx needs no such adjustment: legacy UFL's form compiler estimates its
   own quadrature degree from the form.

Same "compile" and steady-state per-call timing methodology as
ffcx_compare_linear_action.py/ffcx_compare_uflx.py (see the latter's
docstring): compile_seconds covers the WHOLE UFLx side (form ->
quadrature/geometry/tabulation lowering -> MLIR generation -> JIT) for a
fair comparison against FFCx's compile_forms() (UFL analysis + C codegen +
a real C compile), and per-call numbers reuse pre-built buffers/descriptors
on both sides throughout.

Run (same venv, fenics-ffcx installed):
    python3 demo/ffcx_compare_mass_linear_action.py        # degree 1 (default)
    python3 demo/ffcx_compare_mass_linear_action.py 0      # DG0
    python3 demo/ffcx_compare_mass_linear_action.py 3 50000  # override N_CALLS
"""
...
from uflx import Coefficient, TestFunction, coordinate_element, dx, function_space, inner
...
def build_coefficient_mass_form(degree: int):
    """`inner(w, v) * dx` for a Coefficient w on a P{degree} Lagrange
    tetrahedron space (coordinate map always P1/affine, same as
    ffcx_compare_uflx.py's build_stiffness_form). degree=0 requests a
    discontinuous element -- see this module's own docstring point 1.
    Returns (form, ndofs).
    """
    e = element(
        "Lagrange",
    ...
    return inner(w, v) * dx, e.dim


def build_legacy_ufl_form(degree: int):
    """The same form as build_coefficient_mass_form, but expressed in
    legacy UFL -- what FFCx actually consumes. Must use the same Lagrange
    variant (equispaced) and the same discontinuous=(degree == 0) choice as
    the UFLx side -- see ffcx_compare_uflx.py's build_legacy_ufl_form
    docstring for why the variant matters starting at degree 3, and this
    module's own docstring point 1 for why degree 0 needs discontinuous.
    """
    ...
    return ufl.inner(w, v) * ufl.dx


def ffcx_compile(degree: int, w_dofs: np.ndarray):
    """Returns (compile_seconds, fast_call, vec). Same shape as
    ffcx_compare_linear_action.py's ffcx_compile() -- see that file's
    docstring for the tabulate_tensor argument order this follows.
    """
...
def _mlir_fast_calls(engine, kernel_name: str, ndofs: int, w_dofs: np.ndarray):
    """Identical to ffcx_compare_linear_action.py's _mlir_fast_calls (same
    dual 'invoke' / 'direct_ctypes' variant pattern, same three-descriptor
    calling convention: the mass form has exactly one Coefficient too).
    """
    vec = np.zeros(ndofs, dtype=np.float64)
    w = np.ascontiguousarray(w_dofs, dtype=np.float64)
...
def uflx_compile(degree: int, w_dofs: np.ndarray, pipeline: str | None = None):
    ...
    Passes degree + 1, not the bare element degree, as
    generate_mlir_module's own `degree` (quadrature-sizing) argument: that
    function ultimately sizes its quadrature rule via
    uflx_mlir.lowering.lower_form as `max(2*(degree - 1), 1)`, tuned for a
    gradient-square (stiffness) integrand, so `degree + 1` there gives
    `2*degree` -- exactly what a mass integrand phi_i*phi_j needs. See this
    module's own docstring point 2 and generate_kernel.reference_mass's
    docstring for the same fix spelled out from the reference side, and
    where this was checked against lower_form's own emitted quadrature
    table sizes.

    pipeline defaults to mlir_harness.UFLX_PIPELINE; pass
    mlir_harness.UFLX_OPTIMIZED_PIPELINE to additionally run
    canonicalize/cse/loop-invariant-code-motion first.
    """
    pipeline = pipeline if pipeline is not None else mlir_harness.UFLX_PIPELINE
    form, ndofs = build_coefficient_mass_form(degree)
    kernel_name = f"tabulate_tensor_p{degree}_coefficient_mass_uflx"
...
def run_variant(label: str, compile_fn, degree: int, n_calls: int, vec_ref: np.ndarray):
    ...
    for variant, fast_call in fast_calls.items():
        ...
        # rezeroing, same as ffcx_compare_linear_action.py.
        vec.fill(0.0)
        fast_call()
...
# Same baseline/licm pipeline comparison as ffcx_compare_linear_action.py --
# see ffcx_compare_uflx.py's PIPELINE_VARIANTS docstring for why
# UFLX_OPTIMIZED_PIPELINE exists (kept for historical comparison;
# ../uflx_mlir/hoist.py's own fission/hoisting analysis is what actually
# closes the gap now, MLIR's generic LICM doesn't rediscover it).
PIPELINE_VARIANTS = [
...
def main():
    ...
    mass_ref = generate_kernel.reference_mass(COORDS, degree)
    vec_ref = mass_ref @ w_dofs
    ...
    print(flush=True)
    print(
        "Linear action of inner(w, v)*dx for a Coefficient w -- the local tensor is a",
        flush=True,
    )
    print(
        "length-ndofs vector (mass_matrix @ w's dof vector), not a matrix, and both generated",
        flush=True,
    )
    print(
        "kernels take a third coefficients-buffer argument alongside the local tensor and cell",
        flush=True,
    )
    print(
        "coordinates -- see ffcx_compare_linear_action.py, this file's stiffness-operator",
        flush=True,
    )
    print(
        "sibling, for that shared methodology. At degree 0, w is a DG0 (cellwise-constant)",
        flush=True,
    )
    print(
        "Coefficient with a single dof: unlike the stiffness operator (whose action on a DG0",
        flush=True,
    )
    print(
        "Coefficient is exactly zero, see uflx/operators.py), the mass operator's action is a",
        flush=True,
    )
    print(
        "genuine nonzero result, so degree 0 here is a real numerical check of the DG0",
        flush=True,
    )
    print(
        "Coefficient-summation path through the full MLIR JIT pipeline, not just that its",
        flush=True,
    )
    print(
        "gradient vanishes. [baseline] uses UFLX_PIPELINE (plain dialect conversion); [licm]",
        flush=True,
    )
    print(
        "additionally runs canonicalize/cse/loop-invariant-code-motion first -- compare the two",
        flush=True,
    )
    print("rows to see whether that closes any remaining gap.", flush=True)
