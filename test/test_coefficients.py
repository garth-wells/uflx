...
def test_coefficient_explicit_count(lagrange_element):
    """Test that an explicitly given count is used instead of auto-generating one."""
    element = lagrange_element("triangle", 1)
    domain = coordinate_element(lagrange_element("triangle", 1, (2,)))
    space = function_space(domain, element)

    w = Coefficient(space, count=42)
    assert w.count == 42
